#include "persistence/storage/exampleServiceConfigStore.h"
...
ExampleServiceConfigStore::ExampleServiceConfigStore(std::string configPath)
    : configPath_(std::move(configPath))
{
    if (configPath_.empty()) {
        throw std::invalid_argument("example service config path is required");
    }
}

mlcp::hmi::service::example::ExampleServiceConfig ExampleServiceConfigStore::load(
    const mlcp::hmi::service::example::ExampleServiceConfig& defaultConfig) const
{
    ...
    config.lowerLimit = readDouble(values, "lowerLimit", config.lowerLimit);
    config.upperLimit = readDouble(values, "upperLimit", config.upperLimit);
...
void ExampleServiceConfigStore::save(
    const mlcp::hmi::service::example::ExampleServiceConfig& config) const
{
    const fs::path path = configPath_;
    ...
    const fs::path temporaryPath = path.string() + ".tmp";
    {
        ...
        output << "lowerLimit=" << config.lowerLimit << '\n';
        output << "upperLimit=" << config.upperLimit << '\n';
...
const std::string& ExampleServiceConfigStore::configPath() const
{
    return configPath_;
}

std::string ExampleServiceConfigStore::defaultConfigPath()
{
    if (const char* configDirectory = std::getenv("MLCP_HMI_CONFIG_DIR")) {
        return (fs::path(configDirectory) / "example_service.conf").string();
    }

    if (const char* homeDirectory = std::getenv("HOME")) {
        return (fs::path(homeDirectory) / ".config" / "mlcp-hmi" /
                "example_service.conf").string();
    }

    return (fs::temp_directory_path() / "mlcp-hmi" / "example_service.conf").string();
}
