...
void AppWiring::initializeServices(
    ...
    const LifecycleReporter& serviceThreadReporter)
{
    ...
    try {
        ...
        serviceRegistry_.add(ServiceRegistration{
            &*tempService_,
...
void AppWiring::configureSelfTestChecks(
...
void AppWiring::startServices()
{
    serviceHost_.startAll();
}

void AppWiring::stopServices()
{
    serviceHost_.stopAll();
}

void AppWiring::clearServices()
{
    serviceRegistry_.clear();
    serviceHost_.clear();
    tempService_.reset();
...
mlcp::hmi::service::temp::TempServiceConfig AppWiring::tempServiceConfig()
{
    if (!tempService_.has_value()) {
...
mlcp::hmi::service::temp::TempServiceSnapshot AppWiring::tempServiceSnapshot()
{
    if (!tempService_.has_value()) {
        throw std::runtime_error("temp service is unavailable");
    }
...
void AppWiring::applyTempServiceConfig(
...
void AppWiring::reportTempServiceState(
